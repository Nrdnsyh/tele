...
def stop_dialog():
    markup = types.ReplyKeyboardMarkup(resize_keyboard = True)
    item1 = types.KeyboardButton('🗣 Beri tahu profil Anda')
...
def start(message):
    ...
    bot.send_message(message.chat.id, 'Halo {0.first_name}! Selamat datang di obrolan anonim! Tunjukkan jenis kelamin Anda! '.format(message.from_user), reply_markup = markup)
...
def stop(message):
    ...
    if chat_info != False:
        ...
        markup = types.ReplyKeyboardMarkup(resize_keyboard = True)
        item1 = types.KeyboardButton('✏️ Dialog selanjutnya')
